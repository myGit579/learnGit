...
def get_require_verbatim(zip):
  try:
    data = zip.read("verbatim_file")
  except KeyError:
    print( "Warning: could not find verbatim_file in %s." % zip)
    data = ""

#  dat = data.decode(encoding='utf-8')
  verbatim = []
...
def Run(args, **kwargs):
  """Create and return a subprocess.Popen object, print(ing the command
...
def LoadInfoDict(zip, type):
  ...
  d = {}
  try:
    for line in zip.read("META/misc_info.txt").split(b"\n"):
      line = line.strip()
  ...
  d["fstab"], partitions = LoadRecoveryFSTab(zip, type)
  return d, partitions
...
def LoadRecoveryFSTab(zip, type):
  ...
  try:
    ...
    print( "so, we can read ./fstab.txt")
    f = open("fstab.txt", "rb")
    data = f.read()
...
def GetBootableImage(name, prebuilt_name, unpack_dir, tree_subdir, partition, ftype):
  """Return a File object (with name 'name') with the desired bootable
  image.  Look for it in 'unpack_dir'/MTD under the name
  'prebuilt_name', otherwise construct it from the source files in
  'unpack_dir'/'tree_subdir'."""
  prebuilt_path = os.path.join(unpack_dir, partition.pkgpath[1:], prebuilt_name)
  if os.path.exists(prebuilt_path):
    print( "using prebuilt %s..." % (prebuilt_name,))
    print( "using prebuilt %s..." % (prebuilt_path,))
    return File.FromLocalFile(name, prebuilt_path, ftype)
  else:
    print( "error: building image from target_files %s..." % (tree_subdir,))
...
def UnzipTemp(filename, pattern=None):
  ...
  temp dir, then unzip bar.zip into that_dir/MTD.
  ...
  if m:
    ...
    unzip_to_dir(m.group(2), os.path.join(tmp, "MTD"))
    filename = m.group(1)
  else:
...
def CheckSize(data, target, info_dict, tp):
...
def ReadApkCerts(tf_zip):
  ...
  certmap = {}
  for line in tf_zip.read("META/apkcerts.txt").split(b"\n"):
    line = line.strip()
    if not line: continue
...
class PasswordManager(object):
  ...
  def UpdateAndReadFile(self, current):
    ...
    first_line = None
    sorted = [(not v, k, v) for (k, v) in current.items()]
    sorted.sort()
...
def ZipWriteStr(zip, filename, data, perms=0o644):
...
class File(object):
  ...
  @classmethod
  def FromLocalFile(cls, name, diskname, ftype):
...
#      print(stemp.name, ttemp.name, ptemp.name)
      p = Run(["free"])
      p.communicate()
      ...
      #p = Run(cmd)
#      p = Run(cmd,shell=True , stdout=subprocess.PIPE, stderr=subprocess.PIPE)
      _, err = p.communicate()
      if err or p.returncode != 0:
        print( "WARNING: failure running %s:\n%s\n" % (diff_program, err))
        return None
      #print( "afterRun xdelta-------------: \n")
      #p = Run(["free"])
      #p.communicate()
      diff = ptemp.read()
    finally:
...
#  by_size = sorted(by_size,reverse=True)
