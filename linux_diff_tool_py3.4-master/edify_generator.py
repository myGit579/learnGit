...
class EdifyGenerator(object):
  ...
  def ApplyPatch(self, srcfile, tgtfile, tgtsize, tgtsha1, *patchpairs):
    ...
    cmd = "".join(cmd)
    cmd = "assert("+cmd+");"
  #  print ("script_ApplyPatch_cmd = ",cmd)
    self.script.append(self._WordWrap(cmd))
  ...
  def WriteRawImage(self, mount_point, fn):
    ...
    if fstab:
      p = fstab[mount_point]
      partition_type = p.fs_type
#      args = {'device': p.device, 'fn': fn}
      args = {'device': fn, 'fn': p.device}
  ...
  def AddToZip(self, input_zip, output_zip, fota, input_path):
    ...
    if input_path is None:
      ...
      data = open(os.path.join(input_path, "updater"),'rb').read()
      if fval:
